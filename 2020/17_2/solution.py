...
class Simulation(object):
    def __init__(self, parsed_data):
        cubes, dim_y, dim_x = parsed_data
        self.cubes = cubes
        self.dim_y = dim_y
        self.dim_x = dim_x

    def calc_adjacent_space(self, cubes, w, z, y, x):
        return sum([1 if cubes.get(index) == STATE_ACTIVE else 0
                    for index in get_neighbours(w, z, y, x)
                    ])
    ...
    def simulate_step(self, iteration, cubes):
        new_state = {}
        for w in range(-iteration, iteration + 1):
...
def preprocess(raw_data):
    ...
    for y, line in enumerate(lines):
        for x, cube in enumerate(line):
            pocketdimension[(w, z, y, x)] = cube
    return (pocketdimension, dim_y, dim_x)
