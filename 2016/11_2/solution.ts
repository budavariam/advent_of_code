...
class Optimizer {
    ...
    constructor(input: string[]) {
        ...
        parsed.generator.set("d", 0);
        parsed.microchip.set("d", 0);
        parsed.generator.set("e", 0);
        parsed.microchip.set("e", 0);
...
class State {
    ...
    public isSafe(): boolean {
        ...
        const result = !microchips
        .some(([chipName, chipFloor]) => (chipFloor !== (this.generator.get(chipName) || 0)) &&
    ...
    public hash() {
        // filter out the same states to avoid exponential growth because
        // the pairs in the same state with different names are interchangable.
        const flattenedHash = Array
            .from(this.generator)
            .map(([genName, genFloor]) => "" + genFloor + (this.microchip.get(genName) || 0))
            .sort()
            .join("");
        return "" + this.elevator + flattenedHash;
    }
    ...
    public getNextValidStates(maxFloor: number, visited: Set<string>): State[] {
        ...
        return [
            ...
            ...this.moveDownTwoElements(maxFloor, visited),
        ];
    }
    ...
    private moveDownTwoElements(maxFloor: number, visited: Set<string>): State[] {
        const result: State[] = [];
        if (this.elevator > 0) {
            const sameFloorItems = [
                ...Array.from(this.microchip)
            ...
            for (const [first, second] of combinationIDs) {
                const [firstName, firstType] = sameFloorItems[first];
                const [secondName, secondType] = sameFloorItems[second];
                const newState = this.moveDuelDown(maxFloor, visited, firstName, firstType, secondName, secondType);
                if (newState) {
                    result.push(newState);
                }
            }
