export function solution(input: string[], options?: any) {
    ...
    return "" + pathFinder.countPaths(start, limit);
}
...
class Labyrinth {
    ...
    public countReachedStates() {
        let result = 0;
        for (const [_, field] of Array.from(this.map.entries())) {
...
class PathFinder {
    ...
    public countPaths(start: Path, limit: number): number {
        this.queue.push(start);
        while (this.queue.length > 0) {
            const currentPath: Path = this.queue.shift() as Path;
            if (currentPath.distance > limit) {
                return this.map.countReachedStates();
            }
