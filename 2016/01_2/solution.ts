...
function processData(parsed: Direction[]): CoordNumbers {
    ...
    for (const element of parsed) {
        const notFoundIntersection = position.move(element);
        if (!notFoundIntersection) {
            break;
        }
    }
...
class Position {
    ...
    public move(newDir: Direction) {
        ...
        return this.adjustPosition(newDir.count);
    }
    ...
    private adjustPosition(count: number) {
        ...
        if (moveBy[0] === "x") {
            for (let i = 0; i < count; i++) {
                const currentPosition = `${this.x}_${this.y}`;
                if (!this.visited.has(currentPosition)) {
                    this.x += moveBy[1];
