...
fn can_split_into_three(items: &[u64], target: u64) -> bool {
    for size1 in 1..=items.len() {
        for combo1_indices in (0..items.len()).combinations(size1) {
            let sum1: u64 = combo1_indices.iter().map(|&i| items[i]).sum();
            if sum1 != target {
                continue;
            }

            let remaining: Vec<u64> = (0..items.len())
                .filter(|i| !combo1_indices.contains(i))
                .map(|i| items[i])
                .collect();

            if can_split_into_two(&remaining, target) {
                return true;
            }
        }
    }
...
fn solution(input: &str) -> String {
    let numbers: Vec<u64> = input
        ...
        .collect();
    let bin_size: u64 = numbers.iter().sum::<u64>() / 4;
    ...
    for bin_1_size in 1..=numbers.len() {
        for combo1_indices in (0..numbers.len()).combinations(bin_1_size) {
            ...
            if can_split_into_three(&rest_items, bin_size) {
