...
func (c *cartography) countStillWaterInVisibleRange() int {
	result := 0
	for place, v := range c.carta {
		if place[1] >= c.minY && place[1] <= c.maxY && (v == typeStillwater) {
			result++
		}
	}
	return result
...
func ReservoirResearch(data []string) int {
	...
	result := ground.countStillWaterInVisibleRange()
	// ground.printMap()
	return result
}
