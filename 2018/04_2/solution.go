...
func main() {
	data := utils.LoadInput("04_1/input.txt")
...
func ChooseGuardAndMinute(data []string) int {
	...
	prevMinute, maxSleepTime, selectedGuard, maxMinute := -1, -1, -1, -1
	for _, line := range parsedLines {
		if line.wakesUp {
			for setMinute := prevMinute; setMinute < line.minute; setMinute++ {
				timeline[line.guardID][setMinute]++
				if maxSleepTime < timeline[line.guardID][setMinute] {
					selectedGuard = line.guardID
					maxSleepTime = timeline[line.guardID][setMinute]
					maxMinute = setMinute
				}
			}
		}
