...
func main() {
	input := utils.LoadInput("24_2")
	result := IncreaseFind(input)
	fmt.Println(result)
}

//IncreaseFind finds the smallest amount of boost that the immune system needs
func IncreaseFind(input []string) int {
	i := 0
	prevwon := false
	for {
		won, remainingUnits := ImmuneSystemSimulator(input, i)
		if !prevwon && won {
...
// ImmuneSystemSimulator simulates the special reindeer immune system with a little boost
func ImmuneSystemSimulator(input []string, boost int) (bool, int) {
	battle := parseInput(input, boost)
	sumUnitsKilledLastTime, sumUnitsKilled := -1, -1
	for !battle.isFightOver() {
		targets := battle.targetSelectionPhase()
		sumUnitsKilled = battle.attackPhase(targets)
		if sumUnitsKilled == 0 && sumUnitsKilledLastTime == 0 {
			return false, -1
		}
		sumUnitsKilledLastTime = sumUnitsKilled
	}
	return len(battle.teams[0]) > 0, battle.countWinningArmyUnits()
}

func parseInput(input []string, boost int) battleGround {
...
func (bg *battleGround) attackPhase(targets targetMapping) int {
	...
	for _, attacker := range units {
		if tgt, has := targets[attacker]; has {
			...
			survived, unitKilled := tgt.enemy.takeDamage(damage)
			sumUnitsKilled += unitKilled
			if !survived {
