...
impl PaintingRobot {
    fn build(input: &str) -> Self {
        ...
        PaintingRobot { cpu: machine, direction: Direction::North, position: (0,0) }
    }
    ...
    fn step(&mut self, world: &mut HashMap<Position, Color>) -> bool {
        ...
        if !finished {
            ...
            world.entry(self.position).and_modify(|x| *x = color_to_paint).or_insert(color_to_paint);
...
fn solution(input: &str) -> String {
    let mut world: HashMap<Position, Color> = HashMap::new();
